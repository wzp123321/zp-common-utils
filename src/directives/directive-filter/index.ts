...
const inputFilerInstance = {
  created(el: HTMLInputElement, binding: DirectiveBinding<any>) {
    addEventByType(el, binding);
  },
  mounted(el: HTMLInputElement, binding: DirectiveBinding<any>) {
    el.value = filter(el, binding);
  },
  beforeUpdate(el: HTMLInputElement, binding: DirectiveBinding<any>) {
    addEventByType(el, binding);
  },
};

export const VInputFilter = {
  install(app: App) {
    app.directive('inputFilter', inputFilerInstance);
  },
  directive: inputFilerInstance,
};
